...
\begin{center}
\large{\bf Evaluación de la fusión de rangos (\textit{rank fusion}) frente a modelos de reordenamiento neuronal (\textit{neural reranking}) en la recuperación de información en español: un análisis con modelos abiertos sobre el conjunto de datos \textsc{MessIRve}}

\normalsize{Tesis de Licenciatura}\\
\vspace{0.5cm}

\normalsize{\bf Jorge Esteban Mendoza Ortiz}\\
...
\end{center}
% «center» deja armado en \everypar el «sin sangría» del párrafo siguiente
% (\@doendpe hace \setbox\z@\lastbox). Se vacía para conservar la sangría,
% que es la convención del documento (ver «Formato de párrafo» en tesis.tex).
\everypar{}%
